...
saftest_map_lock_status_to_string(SaLckLockStatusT op)
{
    ...
    } else if (SA_LCK_LOCK_TIMED_OUT == op) {
...
add_lock_resource()
{
    ...
    saftest_list_element_create(lock_list, res);
...
delete_lock_resource(lock_resource_t *res)
{
    ...
    for (element = saftest_list_first(lock_list);
         NULL != element;
    ...
    saftest_msg_t **reply)
{
    ...
    lck_res = lookup_lock_resource_from_request(request);

    saftest_assert(0 == lck_res->lck_handle,
                   "Resource must be finalized before deletion\n");

    delete_lock_resource(lck_res);
    ...
    saftest_msg_t **reply)
{
    ...
    (*reply) = saftest_reply_msg_create(request, map_entry->reply_op,
                                        SA_AIS_OK);
    saftest_msg_set_ubit32_value((*reply), "NUM_LCK_RESOURCES",
                                 saftest_list_size(lock_list));
    for (ndx = 0, element = saftest_list_first(lock_list);
         NULL != element;
         ndx++, element = saftest_list_next(element)) {
        res = (lock_resource_t *)element->data;
        sprintf(key, "LCK_RESOURCE_%d_ID", ndx);
        saftest_msg_set_ubit32_value((*reply), key, res->lock_resource_id);
        sprintf(key, "LCK_RESOURCE_%d_DISPATCH_FLAGS", res->lock_resource_id);
        saftest_msg_set_str_value((*reply), key,
                                  saftest_dispatch_flags_to_string(
                                      res->dispatch_flags));
        sprintf(key, "LCK_RESOURCE_%d_LONG_LIVED", res->lock_resource_id);
        if (res->long_lived) {
    ...
    saftest_msg_t **reply)
{
    ...
    strcpy(((char *)lock_res->lock_name.value),
           saftest_msg_get_str_value(request, "LOCK_NAME"));
    ...
    saftest_msg_t **reply)
{
    ...
    status = saLckResourceLock(lock_res->res_handle,
                               lock_id,
                               lock_mode,
                               lock_flags,
    ...
    saftest_msg_t **reply)
{
    ...
    lock_res->lock_status = 0;
    status = saLckResourceLockAsync(lock_res->res_handle, invocation, lock_id, 
...
void
saftest_daemon_handle_incoming_lock_message(void *data, void *key)
{
...
void saftest_daemon_add_lock_resource_to_fdset(void *data, 
                                                void *key)
{
    lock_resource_t *lock_res;
    ...
    fd_set *except_fd_set)
{
    ...
    saftest_list_each(lock_list, saftest_daemon_add_lock_resource_to_fdset,
                      &set_key);
...
saftest_daemon_check_fds(
    ...
    fd_set *except_fd_set)
{
    saftest_list_each(lock_list, saftest_daemon_handle_incoming_lock_message,
                      read_fd_set);
}
    ...
    saftest_msg_t *request)
{
    ...
    status = saftest_reply_msg_get_status(reply);
    if (SA_AIS_OK == status) {
        saftest_log("NUM_LCK_RESOURCES=%d\n",
                    saftest_msg_get_ubit32_value(reply,
                                                 "NUM_LCK_RESOURCES"));
        for (ndx = 0;
             ndx < saftest_msg_get_ubit32_value(reply, "NUM_LCK_RESOURCES");
             ndx++) {
            sprintf(key, "LCK_RESOURCE_%d_ID", ndx);
            lock_resource_id = saftest_msg_get_ubit32_value(reply, key);
            saftest_log("LCK_RESOURCE_%d_ID=%d\n",
                        ndx, lock_resource_id);
            sprintf(key, "LCK_RESOURCE_%d_DISPATCH_FLAGS", lock_resource_id);
            saftest_log("LCK_RESOURCE_%d_DISPATCH_FLAGS=%s\n",
