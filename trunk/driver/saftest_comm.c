...
void
saftest_msg_free_kvp(gpointer data, gpointer user_data)
{
...
saftest_msg_free(saftest_msg_t **msg)
{
    ...
    if ((NULL != msg) && (NULL != *msg)) {
        g_list_foreach((*msg)->internal_key_value_pairs, saftest_msg_free_kvp,
                       NULL);
        g_list_free((*msg)->internal_key_value_pairs);
        g_list_foreach((*msg)->external_key_value_pairs, saftest_msg_free_kvp,
                       NULL);
        g_list_free((*msg)->external_key_value_pairs);
                                  ...
                                  const char *key, const char *value, int flags)
{
    ...
    assert(g_list_length(*list) < SAFTEST_MSG_MAX_KEY_VALUE_PAIRS);
...
saftest_msg_get_internal_ubit64_value(saftest_msg_t *msg, const char *key)
{
    ...
    value = strtoull(kvp->value, NULL, 0);
    return(value);
}
*/
...
void
saftest_flat_msg_add_internal_kvp(gpointer data, gpointer user_data)
{
    ...
    kvp = (saftest_key_value_pair_t *)data;
    sfm = (saftest_flat_msg_t *)user_data;
...
void
saftest_flat_msg_add_external_kvp(gpointer data, gpointer user_data)
{
    saftest_flat_msg_t *sfm;
    saftest_key_value_pair_t *kvp = NULL;
...
saftest_create_flat_msg_from_saftest_msg(saftest_msg_t *msg)
{
    ...
    g_list_foreach(msg->internal_key_value_pairs, 
                   saftest_flat_msg_add_internal_kvp, sfm);
    g_list_foreach(msg->external_key_value_pairs, 
                     ...
                     saftest_msg_t *request, saftest_msg_t **reply)
{
    ...
    bytes_sent = send(fd, sfm, sizeof(saftest_flat_msg_t), MSG_NOSIGNAL);
...
saftest_recv_flat_msg(int fd)
{
    ...
    for (counter = 0; 
        ...
        bytes_recvd = recv(fd, 
                           ...
                           bytes_left, MSG_WAITALL | MSG_NOSIGNAL);
